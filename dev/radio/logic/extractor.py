...
def analyze_wav(wav_path, output_json_path):
    ...
    # Peak normalization & log-like scaling across the whole song
    # Use the 95th percentile band value as the scaling peak (to ignore rare transients)
    ...
    for frame in raw_frames:
        ...
        for val in frame:
            if val <= 0:
                ...
                # Apply square root scaling for visual compression (brings up quiet sounds)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 extractor.py <video_id>")
        sys.exit(1)
